...
public class Wildcard {
  ...
  public static boolean isWildcard(String token)
  {
...
//		Vector<String>book_titles  = new Vector();
		...
		Vector<String>wildCards = new Vector();
...
//		query = input.nextLine();
		...
		{
			root[i] = root[i].toLowerCase();
			if(isWildcard(root[i]))
				{
					root[i] = root[i].substring(1,root[i].length()-1);
					wildCards.addElement(root[i]);
		//			System.out.println(root[i]);
				//	break;	
				}
			
			else if(!s.m_Words.contains(root[i]))
			{
	   ...
	   try 
		...
		try
		{
			   ...
			   {
				   String get_wildCards = "SELECT distinct term from revindex where term like '" + wildCards.elementAt(i)+"'";
				   ResultSet rs_wildCards = stmt.executeQuery(get_wildCards);
				   while(rs_wildCards.next())
				   
					   tokens.addElement(rs_wildCards.getString(1));
			   }
	...
	//		   System.out.println(numberOfPages/page_count);
			   ...
			   System.out.println(tokens.elementAt(j));
			}
