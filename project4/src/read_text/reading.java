...
public class reading 
{
			    ...
			    for(int n=0;n<tokens.length;n++)
			    {
			    	if(!(s.m_Words.contains(tokens[n].toLowerCase())))
			    	{
			    		tokens[n]= seperator(tokens[n]);
			    		 stemmer.setCurrent(tokens[n]);
						  stemmer.stem();
						  tokens[n] = stemmer.getCurrent();
			    		f_array.addElement((tokens[n]).trim().toLowerCase());
  ...
  public String seperator(String token)
  {
	  ...
	  if(root1.length>1)
	  {
	  root1[0] = spcl_char_mul(root1[0]);
	  f_array.addElement(root1[0].toLowerCase());
	  return spcl_char_mul(root1[1]);
	  }
	  else
		  return spcl_char_mul(token);
	  }
  }
